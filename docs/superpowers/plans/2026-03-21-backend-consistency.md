...
**All commands:** prefix with `GOWORK=off`. Working dir: `/path/to/repos/src/go-code/`.
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go build ./... && GOWORK=off go test ./internal/callgraph/ -count=1 -timeout 120s`
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./... -timeout 300s -count=1`
Expected: All 30+ packages pass.

- [ ] **Step 2: Build Docker image**

Run: `cd ~/deploy/example-server && docker compose build go-code`
