...
Run:
```bash
cd /path/to/repos/src/go-code && go get github.com/jackc/pgx/v5
```
...
Run: `cd /path/to/repos/src/go-code && go mod tidy`
...
func TestGraphName(t *testing.T) {
	name := graphName("/path/to/repos/src/go-code")
	if name == "" {
	...
	if graphName("/path/to/repos/src/go-code") != name {
		t.Error("graphName not deterministic")
	}
	// Different input → different output
	if graphName("/path/to/repos/src/go-search") == name {
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run 'TestEscape|TestGraph|TestIsRead'`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run TestTemplate`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run TestParseClas`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run 'TestParseClas|TestClassif'`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run 'TestIsReadOnlyGuard|TestExtractCypher'`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/codegraph/ -v -run 'TestIsFresh|TestBuildBatch|TestBuildEdge'`
...
**Files:**
- Modify: `~/deploy/example-server/docker-compose.yml` (add DATABASE_URL to go-code)
...
```bash
cd ~/deploy/example-server && git add docker-compose.yml
