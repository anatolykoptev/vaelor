...
- **Server side** (`ssh krolik`, `$REPO_ROOT/`): enable `AUTO_INDEX_DIRS` for zero-cold-start semantic search; wire the dormant `internal/learnings` package into `understand` (read) and `review_pr_post` (write); add vector similarity to `Nearest`.
...
- Server: `ssh krolik` → `cd $REPO_ROOT` → `GOWORK=off make test` → `make deploy`.
...
Run: `ssh krolik "cd $REPO_ROOT && GOWORK=off make deploy"`
...
Run: `ssh krolik "cd $REPO_ROOT && GOWORK=off go test ./internal/analyze/..."`
...
```
ssh krolik "cd $REPO_ROOT && GOWORK=off go test ./... && GOWORK=off go build ./..."
```
Expected: pass.
...
Run: `ssh krolik "cat $REPO_ROOT/internal/learnings/schema.sql"`
...
```
ssh krolik "cd $REPO_ROOT && GOWORK=off go test ./internal/learnings/... -tags=integration"
```
...
**Step 1:** `ssh krolik "cd $REPO_ROOT && GOWORK=off make deploy"`
