...
- **Server side** (`ssh krolik`, `/home/krolik/src/go-code/`): enable `AUTO_INDEX_DIRS` for zero-cold-start semantic search; wire the dormant `internal/learnings` package into `understand` (read) and `review_pr_post` (write); add vector similarity to `Nearest`.
...
**Cross-session paths:**
- Client: `/Users/anatoliikoptev/.claude/{commands,hooks,settings.json}` on mac.
- Server: `ssh krolik` → `cd /home/krolik/src/go-code` → `GOWORK=off make test` → `make deploy`.
...
**Files:**
- Create: `/Users/anatoliikoptev/.claude/commands/gc-understand.md`
...
**Files:**
- Create: `/Users/anatoliikoptev/.claude/commands/gc-impact.md`
- Create: `/Users/anatoliikoptev/.claude/commands/gc-pr.md`
- Create: `/Users/anatoliikoptev/.claude/commands/gc-graph.md`
- Create: `/Users/anatoliikoptev/.claude/commands/gc-rewrite.md`
...
**Files:**
- Create: `/Users/anatoliikoptev/.claude/hooks/go-code-prepare-change.sh`
- Modify: `/Users/anatoliikoptev/.claude/settings.json` (register hook)
...
resp=$(curl -sS -X POST https://mcp.krolik.run/code/mcp \
...
```json
{
  ...
  "hooks": [
    { "type": "command", "command": "/Users/anatoliikoptev/.claude/hooks/go-code-prepare-change.sh" }
  ]
}
```
...
**Files:**
- Create: `/Users/anatoliikoptev/.claude/hooks/go-code-auto-review-pr.sh`
- Modify: `/Users/anatoliikoptev/.claude/settings.json`
...
```json
{
  "matcher": "Bash",
  "hooks": [
    { "type": "command", "command": "/Users/anatoliikoptev/.claude/hooks/go-code-auto-review-pr.sh" }
  ]
}
```
...
**Files:**
- Modify: `/Users/anatoliikoptev/.claude/projects/-Users-anatoliikoptev/memory/mcp-tools-catalog.md`
...
**Files:**
- Modify: `/home/krolik/src/go-code/deploy/go-code.env` (on `ssh krolik`)

**Step 1: Inspect current env**

Run on krolik: `grep -E 'AUTO_INDEX|PATH_MAPPING' /home/krolik/src/go-code/deploy/go-code.env`
If absent, proceed.
...
Run: `ssh krolik "cd /home/krolik/src/go-code && GOWORK=off make deploy"`
...
Run (after ~60s):
```
ssh krolik "docker logs go-code --since 2m 2>&1 | grep -iE 'auto.?index|indexed'"
```
...
- `repo: "/home/krolik/src/piter-now"`
...
**Files:**
- Modify: `/home/krolik/src/go-code/internal/analyze/deps.go`
- Modify: `/home/krolik/src/go-code/cmd/go-code/register.go`
- Modify: `/home/krolik/src/go-code/cmd/go-code/config.go`

**Step 1: Failing test**

Create `/home/krolik/src/go-code/internal/analyze/deps_learnings_test.go`:
...
Run: `ssh krolik "cd /home/krolik/src/go-code && GOWORK=off go test ./internal/analyze/..."`
...
```
ssh krolik "cd /home/krolik/src/go-code && GOWORK=off go test ./... && GOWORK=off go build ./..."
```
Expected: pass.
...
**Files:**
- Modify: `/home/krolik/src/go-code/cmd/go-code/tool_understand.go`
- Modify: `/home/krolik/src/go-code/internal/compound/understand.go` (extend result with `PriorLearnings []learnings.Record`)

**Step 1: Failing test**

Create `/home/krolik/src/go-code/cmd/go-code/tool_understand_learnings_test.go` — seed a fake Store with one Record for `(repo="r", symbol="Foo")`, call `handleUnderstand`, assert the output contains the Note text under `<prior_learnings>`.
...
**Files:**
- Modify: `/home/krolik/src/go-code/cmd/go-code/tool_review_pr_post.go`
...
**Files:**
- Modify: `/home/krolik/src/go-code/internal/learnings/store.go`
- Modify: `/home/krolik/src/go-code/internal/learnings/schema.sql`

**Step 1: Inspect schema**

Run: `ssh krolik "cat /home/krolik/src/go-code/internal/learnings/schema.sql"`
...
**Step 4: Test against real pgvector on krolik**

```
ssh krolik "cd /home/krolik/src/go-code && GOWORK=off go test ./internal/learnings/... -tags=integration"
```
...
**Files:**
- Create: `/home/krolik/src/go-code/cmd/go-code/learnings_e2e_test.go`
...
**Files:**
- Modify: `/home/krolik/src/go-code/CLAUDE.md` — add env row `LEARNINGS_DATABASE_URL`; one paragraph on the learnings loop.
- Modify: `/home/krolik/src/go-code/README.md` — under Features: "Learnings — prior review verdicts auto-surface in `understand`".
...
**Step 1:** `ssh krolik "cd /home/krolik/src/go-code && GOWORK=off make deploy"`

**Step 2:** From mac: `/gc:understand /home/krolik/src/go-code handleReviewPRPost`
