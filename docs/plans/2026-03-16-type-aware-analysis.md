...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/tier/ -v -count=1`
...
```bash
cd /path/to/repos/src/go-code && git add internal/tier/ && git commit -m "feat: add tier detection system for 3-level analysis capabilities"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/goanalysis/ -v -count=1 -timeout 60s`
Expected: FAIL — package does not exist
...
```bash
cd /path/to/repos/src/go-code && git add internal/goanalysis/ && git commit -m "feat: add Go package loader with go/types support"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/goanalysis/ -run TestResolve -v -count=1 -timeout 60s`
...
```bash
cd /path/to/repos/src/go-code && git add internal/goanalysis/ && git commit -m "feat: add type-aware call resolver using go/types"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/goanalysis/ -run TestConvert -v -count=1 -timeout 60s`
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/goanalysis/ -run "TestConvert|TestMerge" -v -count=1 -timeout 60s`
Expected: PASS (2 tests)

- [ ] **Step 5: Commit**

```bash
cd /path/to/repos/src/go-code && git add internal/goanalysis/ && git commit -m "feat: add TypedEdge to CallGraph converter and graph merger"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/callgraph/ -run "TestBuildFromRepo_Go|TestBuildFromRepo_Fallback" -v -count=1 -timeout 60s`
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/callgraph/ -run "TestBuildFromRepo" -v -count=1 -timeout 60s`
Expected: PASS (2 tests)

- [ ] **Step 5: Run all existing tests to verify no regressions**

Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./... -count=1 -timeout 120s`
Expected: All existing tests pass

- [ ] **Step 6: Commit**

```bash
cd /path/to/repos/src/go-code && git add internal/callgraph/ && git commit -m "feat: integrate go/types resolution into BuildFromRepo with graceful fallback"
```
...
```bash
cd /path/to/repos/src/go-code && git add cmd/go-code/ internal/callgraph/ && git commit -m "feat: add tier and provenance metadata to call_trace, impact, dead_code outputs"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/compound/ -v -count=1`
Expected: FAIL — package does not exist
...
```bash
cd /path/to/repos/src/go-code && git add internal/compound/ cmd/go-code/tool_understand.go cmd/go-code/register.go && git commit -m "feat: add understand compound tool — aggregates symbol info, callees, callers"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./internal/compound/ -run TestPrepareChange -v -count=1`
...
```bash
cd /path/to/repos/src/go-code && git add internal/compound/ cmd/go-code/tool_prepare_change.go cmd/go-code/register.go && git commit -m "feat: add prepare_change compound tool — aggregates impact analysis + dead code check"
```
...
Run: `cd /path/to/repos/src/go-code && GOWORK=off go mod tidy`

- [ ] **Step 2: Run lint**

Run: `cd /path/to/repos/src/go-code && GOWORK=off make lint`
Expected: Clean or fix any issues

- [ ] **Step 3: Run full test suite**

Run: `cd /path/to/repos/src/go-code && GOWORK=off go test ./... -count=1 -race -timeout 120s`
...
```bash
cd /path/to/repos/src/go-code && git add -A && git commit -m "chore: lint, tidy, update docs for v1.18"
```
...
Test via MCP: `understand` tool with `repo=/path/to/repos/src/go-code`, `symbol=BuildCallGraph`
Expected: Returns symbol info + callees + tier

- [ ] **Step 5: E2E test — prepare_change tool**

Test via MCP: `prepare_change` tool with `repo=/path/to/repos/src/go-code`, `symbol=ParseFile`
Expected: Returns impact analysis + dead code status + tier

- [ ] **Step 6: E2E test — call_trace with tier info**

Test via MCP: `call_trace` with `repo=/path/to/repos/src/go-code`, `symbol=BuildCallGraph`, `compact=true`
