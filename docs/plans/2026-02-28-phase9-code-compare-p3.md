...
**Files:**
- Modify: `/path/to/repos/src/go-code/go.mod` (add gum dependency)
- Create: `/path/to/repos/src/go-code/internal/compare/astconv.go`
- Create: `/path/to/repos/src/go-code/internal/compare/astconv_test.go`

**Step 1: Add the gum dependency**

Run: `cd /path/to/repos/src/go-code && go get github.com/smacker/gum`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/compare/ -run TestToGumTree -v`
...
sudo -u example git add go.mod go.sum internal/compare/astconv.go internal/compare/astconv_test.go
sudo -u example git commit -m "feat(compare): add gum dependency and ToGumTree converter
...
**Files:**
- Create: `/path/to/repos/src/go-code/internal/compare/astdiff.go`
- Create: `/path/to/repos/src/go-code/internal/compare/astdiff_test.go`
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/compare/ -run TestComputeASTDiff -v`
...
```bash
cd /path/to/repos/src/go-code
sudo -u example git add internal/compare/astdiff.go internal/compare/astdiff_test.go
sudo -u example git commit -m "feat(compare): add AST diff for modified symbol pairs
...
**Files:**
- Modify: `/path/to/repos/src/go-code/internal/compare/compare.go:44-52` (add DiffSummary to SymbolMatch)
- Modify: `/path/to/repos/src/go-code/internal/compare/compare.go:217-264` (compute AST diff for modified matches)
- Modify: `/path/to/repos/src/go-code/internal/compare/compare_test.go`

**Step 1: Write the failing test**

Add to `/path/to/repos/src/go-code/internal/compare/compare_test.go`:
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/compare/ -run TestAnnotateASTDiffs -v`
Expected: FAIL — `annotateASTDiffs` undefined, `Diff` field doesn't exist

**Step 3: Add Diff field to SymbolMatch**

In `/path/to/repos/src/go-code/internal/compare/compare.go`, add to `SymbolMatch`:
...
In `/path/to/repos/src/go-code/internal/compare/compare.go`:
...
```bash
cd /path/to/repos/src/go-code
sudo -u example git add internal/compare/compare.go internal/compare/compare_test.go
sudo -u example git commit -m "feat(compare): wire AST diff into SymbolMatch and CompareResult
...
**Files:**
- Modify: `/path/to/repos/src/go-code/internal/compare/context.go:151-166` (writePair — include diff summary)
- Modify: `/path/to/repos/src/go-code/internal/compare/context_test.go`

**Step 1: Write the failing test**

Add to `/path/to/repos/src/go-code/internal/compare/context_test.go`:
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/compare/ -run TestBuildCompareContext_IncludesDiffSummary -v`
Expected: FAIL — no "Structural changes" in output

**Step 3: Update writePair to include diff summary**

In `/path/to/repos/src/go-code/internal/compare/context.go`, modify the `writePair` function to add a diff section at the end:
...
```bash
cd /path/to/repos/src/go-code
sudo -u example git add internal/compare/context.go internal/compare/context_test.go
sudo -u example git commit -m "feat(compare): show AST diff in LLM context for modified pairs
...
**Files:**
- Create: `/path/to/repos/src/go-code/internal/compare/importcat.go`
- Create: `/path/to/repos/src/go-code/internal/compare/importcat_test.go`
- Modify: `/path/to/repos/src/go-code/internal/compare/importdiff.go` (add categories to ImportDiff)
- Modify: `/path/to/repos/src/go-code/internal/compare/importdiff_test.go`
- Modify: `/path/to/repos/src/go-code/internal/compare/compare.go` (pass language to ComputeImportDiff)
...
Run: `cd /path/to/repos/src/go-code && go test ./internal/compare/ -run "TestCategorizeImport|TestDetectFrameworks" -v`
...
In `/path/to/repos/src/go-code/internal/compare/importdiff.go`, add fields to `ImportDiff`:
...
In `/path/to/repos/src/go-code/internal/compare/compare.go`, update:
...
In `/path/to/repos/src/go-code/internal/compare/importdiff_test.go`, update all `ComputeImportDiff` calls to pass `"go"` as the third argument:
...
```bash
cd /path/to/repos/src/go-code
sudo -u example git add internal/compare/importcat.go internal/compare/importcat_test.go internal/compare/importdiff.go internal/compare/importdiff_test.go internal/compare/compare.go
sudo -u example git commit -m "feat(compare): add import categorization and framework detection
...
```bash
cd /path/to/repos/src/go-code
sudo -u example git add -A
sudo -u example git commit -m "chore: lint fixes for code_compare P3
