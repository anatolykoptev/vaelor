...
- **NEXT — T2 AGE enrichment (FU-B.2)** remains the next tier when `DATABASE_URL` is set: graph-confirm a route/symbol match via `HANDLES`/`FETCHES`. T1 (offline) is now the baseline default-correct path for acme's non-HTTP coupling.
