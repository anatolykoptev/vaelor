...
func ConvertToCallGraph(typedEdges []TypedEdge, tsSymbols []*parser.Symbol) *callgraph.CallGraph {
	byNameFile, byName := buildSymbolIndexes(tsSymbols)
...
func MergeCallGraphs(tsGraph, typedGraph *callgraph.CallGraph) *callgraph.CallGraph {
	...
	merged := make([]callgraph.CallEdge, len(typedGraph.Edges), len(typedGraph.Edges)+len(tsGraph.Edges))
	...
	// Merge symbol lists deduplicated by name:file.
...
// buildSymbolIndexes creates two lookup maps from a symbol slice:
// - byNameFile: "name:basename(file)" -> symbol (precise match)
// - byName: "name" -> first symbol (fallback for cross-file lookups)
func buildSymbolIndexes(symbols []*parser.Symbol) (byNameFile, byName map[string]*parser.Symbol) {
