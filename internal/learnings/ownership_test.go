...
//
// Requires TWO DSNs since a single-role test can't distinguish "New() ran
// the transfer" from "the connecting role already owned the table":
//   - DATABASE_URL: the role New() connects as (the "app" role).
...
func TestNew_TransfersOwnershipAfterRestore(t *testing.T) {
	...
	if _, err := adminPool.Exec(ctx, "DROP TABLE IF EXISTS review_learnings"); err != nil {
		t.Fatalf("drop review_learnings: %v", err)
	}
	if _, err := adminPool.Exec(ctx, schemaSQL); err != nil {
		t.Fatalf("admin create schema: %v", err)
	}
	ownerBefore := tableOwner(ctx, t, adminPool, "review_learnings")
	...
	ownerAfter := tableOwner(ctx, t, adminPool, "review_learnings")
