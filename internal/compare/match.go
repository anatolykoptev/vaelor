...
// MatchSymbols aligns symbols from two repositories using three passes:
...
//  3. Semantic: delegate remaining unmatched pairs to classifier (if non-nil).
//
...
func MatchSymbols(symbolsA, symbolsB []*parser.Symbol, classifier LLMClassifier) []SymbolMatch {
	...
	// Pass 3: semantic match via LLM classifier.
