...
// leading/trailing whitespace from each part. Internal whitespace is preserved.
...
// parseSchemaStmt classifies a single DDL statement from schemaSQL.
...
// normalizeTable strips a 'public.' schema prefix if present.
...
// runEnsureSchema is the actual schema-init work.
...
func (s *Store) transferOwnership(ctx context.Context, tables map[string]*tableState) {
	for name, st := range tables {
		if st.existed && !st.ownerOK {
			pgutil.TransferOwnership(ctx, s.schema, "designmd", "public."+name)
		}
	}
}
