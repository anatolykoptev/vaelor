...
func main() {
	...
	if secret := os.Getenv("GITHUB_WEBHOOK_SECRET"); secret != "" {
		...
		// Use a method expression (not a method value) so reflect.ValueOf resolves
		// to the actual (*githubWebhookHandler).ServeHTTP function body with real
		// source location, instead of the autogenerated -fm wrapper.
		httpmw.RegisterRoute("POST", "/webhook/github", (*githubWebhookHandler).ServeHTTP)
